...
import (
	...
	data "github.com/onomojo/i18n-country-translations-data"
)
...
func SetDefaultLocale(locale string) error {
	...
	mu.RUnlock()
	if !ok {
		return fmt.Errorf("countries: locale %q not registered", locale)
	}
	mu.Lock()
...
// GetName returns the country name for a code using the default locale.
// The code is automatically uppercased.
func GetName(code string) (string, bool) {
	mu.RLock()
	defer mu.RUnlock()
	if defaultLocale == "" {
		return "", false
	...
	v, ok := m[strings.ToUpper(code)]
	return v, ok
}

// GetNameForLocale returns the country name for a code in a specific locale.
// The code is automatically uppercased.
func GetNameForLocale(locale, code string) (string, bool) {
	mu.RLock()
	defer mu.RUnlock()
...
// GetAllNames returns all country translations for a locale.
func GetAllNames(locale string) (map[string]string, error) {
	mu.RLock()
	defer mu.RUnlock()
	m, ok := translations[locale]
	if !ok {
		return nil, fmt.Errorf("countries: locale %q not registered", locale)
	}
...
func loadLocale(locale string) (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("countries: locale %q not found: %w", locale, err)
	}
	var m map[string]string
	if err := json.Unmarshal(b, &m); err != nil {
		return nil, fmt.Errorf("countries: failed to parse %q: %w", locale, err)
	}
	return m, nil
}
